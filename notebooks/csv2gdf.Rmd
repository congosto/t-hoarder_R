...
nodos  <- tweets %>%
  ...
  mutate(log_followers = ifelse(followers >0, round(log10(followers),0),1 )) %>%
  mutate(log_following = ifelse(following >0, round(log10(following),0),1 )) %>%
  mutate(log_statuses = ifelse(statuses >0,  round(log10(statuses)),0),1 ) %>%
  ...
  left_join(ranking_RT, by = c( "author" = "user_retweeted")) %>%
