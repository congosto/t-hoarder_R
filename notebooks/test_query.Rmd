...
output:
  html_document:
    df_print: paged
params:
    dataset_name : "mercarroba"          # Nombre del dataset
...
num_days <- as.numeric(difftime(end_time ,start_time , units = c("days")))
```
...
```{r tweets_day, echo=TRUE, fig.height=5, fig.width=9, paged.print=FALSE}
tweets_df <- tweets_count %>%
  mutate(start = as.POSIXct(start, "%Y-%m-%d", tz = "Europe/London")) %>%
  mutate(end = as.POSIXct(end, "%Y-%m-%d", tz = "Europe/London" )) %>%
  filter (!(start == end)) %>%
  arrange (start)
...
ggplot() + 
  ...
  scale_y_continuous( 
    ...
    expand= c(0,0)) +
  labs(
    title = paste (base_title , "Tweets by day"),
    x = "",
    y = "Num. Tweets by day",
    color=""
  ) +
...
```{r accumulated tweets, echo=TRUE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
tweets_df <- tweets_count %>%
  mutate(start = as.POSIXct(start, "%Y-%m-%d",tz = "Europe/London")) %>%
  mutate(end = as.POSIXct(end, "%Y-%m-%d", tz = "Europe/London" )) %>%
  filter (!(start == end)) %>%
...
ggplot() + 
  ...
  geom_text_repel(
    data = tweets_df %>% 
      ...
      top_n(1, end),
    aes(x = start, y = cumulative_sum, color = query,
    label = paste0(
      query,
      "(",
      format(
  ...
  scale_y_continuous( 
    ...
    expand= c(0,0)) +
  labs(
    title = paste (base_title , "cumulative tweets by day"),
    x = "",
    y = "Num. cumulative tweets by day",
    color=""
  ) +
  guides(color=guide_legend(ncol=3)) +
  my_theme() +
